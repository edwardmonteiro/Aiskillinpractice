...
- [`docs/functional-skill-architecture.md`](docs/functional-skill-architecture.md) — functional-programming approach for composing skills like pure functions, tracing inputs/outputs, and keeping the catalog portable across IDE and cloud agents (now with a signature template and code sample).
...
- [`skills/`](skills) — lifecycle-organized skills (`consulting`, `design`, `discovery`, `definition`, `delivery`, `data`, `sales`, `run`, `optimization`) with one directory per skill and a `SKILL.md` definition (including `delivery.agent_collaboration` for coordinating Windsurf-style local agents with Devin-style cloud agents). Every skill now includes `Toolchain & Integrations`, `Human Layer: Reflective Practice`, and `Critical Thinking Loop` sections so teams pair automation with self-awareness, emotional intelligence, and adaptive decision-making.
  - Use [`skills/_templates/functional-skill-template.md`](skills/_templates/functional-skill-template.md) when authoring new skills to keep the functional contract (inputs, outputs, idempotence, error handling) consistent.
...
1. **Review artifacts by lifecycle** — Explore each subfolder (`consulting`, `discovery`, `definition`, `delivery`, `data`, `sales`, `run`, `optimization`, `governance`) to see how every skill produces real outputs.
