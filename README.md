...
- [`skills/`](skills) — lifecycle-organized skills (`discovery`, `definition`, `delivery`, `optimization`) with one directory per skill and a `SKILL.md` definition (including `delivery.agent_collaboration` for coordinating Windsurf-style local agents with Devin-style cloud agents). Every skill now includes a `Toolchain & Integrations` section that spells out the Python CLIs, external APIs, and MCP resources teams should use alongside the prompt.
