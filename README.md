...
- [`skills/`](skills) — lifecycle-organized skills (`consulting`, `discovery`, `definition`, `delivery`, `data`, `run`, `optimization`) with one directory per skill and a `SKILL.md` definition (including `delivery.agent_collaboration` for coordinating Windsurf-style local agents with Devin-style cloud agents). Every skill now includes `Toolchain & Integrations`, `Human Layer: Reflective Practice`, and `Critical Thinking Loop` sections so teams pair automation with self-awareness, emotional intelligence, and adaptive decision-making.
