...
- **Python CLI – `scripts/list-skills`**: Reference adjacent governance and optimization skills (skill_release, metric_review) while finalizing notes.
- **Sample artifact – `samples/whatsup-logistics/delivery/release_notes.md`**: Mirror tone and WhatsApp-first messaging from the WhatsUp Logistics release communications.
- **External API – WhatsApp Business Cloud API**: Schedule message templates or broadcast updates to Brazilian courier partners directly from the release output.
- **MCP server – `mcp/servers/skill_executor/server.py`**: Automate changelog extraction or screenshot generation scripts via MCP run requests.
