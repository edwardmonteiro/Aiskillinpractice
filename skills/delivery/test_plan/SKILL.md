...
- **Python CLI – `scripts/list-skills`**: Align the plan with upstream `definition.test_strategy` outputs and downstream optimization reviews.
- **Sample artifact – `samples/whatsup-logistics/delivery/test_plan.md`**: Copy structure and WhatsApp-specific data setup steps from the WhatsUp Logistics plan.
- **External API – BrowserStack & WhatsApp Sandbox APIs**: Provision device matrices and messaging test environments programmatically.
- **MCP server – `mcp/servers/skill_executor/server.py`**: Trigger regression suites or data seeding scripts via MPC-coordinated runs.
