...
- **Python CLI – `scripts/list-skills`**: Pair OKR drafting with adjacent definition skills (metric catalog, story map) before review.
- **Sample artifact – `samples/whatsup-logistics/definition/okr_drafting.md`**: Follow the WhatsUp Logistics cadence for structuring objectives, key results, and initiatives.
- **External API – Google Sheets & Notion OKR APIs**: Push the drafted OKRs directly into planning dashboards for stakeholder review.
- **MCP server – `mcp/servers/skills_catalog/server.py`**: Allow cloud agents to co-draft OKRs within the same workflow as local IDE partners.
