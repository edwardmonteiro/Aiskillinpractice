...
- **Python CLI – `scripts/list-skills`**: Identify complementary definition outputs (OKRs, value proposition) to reference within the map.
- **Sample artifact – `samples/whatsup-logistics/definition/story_map.md`**: Follow the WhatsUp Logistics layout for activities, tasks, and slice sequencing.
- **External API – FigJam/Miro REST APIs**: Push generated story map tables into collaborative whiteboards for workshops.
- **MCP server – `mcp/servers/skills_catalog/server.py`**: Allow remote facilitators to invoke the story map prompt without cloning the repo.
