...
- **Python CLI – `scripts/list-skills`**: Cross-reference discovery problem framing and market scan outputs while refining messaging.
- **Sample artifact – `samples/whatsup-logistics/definition/value_proposition.md`**: Borrow narrative flow and proof-point structuring from the WhatsUp Logistics example.
- **External API – HubSpot/CRM REST endpoints**: Fetch testimonial quotes and pipeline metrics to use as proof points.
- **MCP server – `mcp/servers/skills_catalog/server.py`**: Allow cloud-based marketing agents to invoke the value proposition skill alongside local product partners.
