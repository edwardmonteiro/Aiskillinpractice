...
- **Python CLI – `scripts/list-skills`**: Stitch insights from test_plan, postmortem, and metric_review outputs into the quality narrative.
- **Sample artifact – `samples/whatsup-logistics/optimization/quality_report.md`**: Reuse structure and WhatsApp courier indicators from the WhatsUp Logistics report.
- **External API – Jira & TestRail REST APIs**: Pull defect counts, automation results, and coverage stats automatically.
- **MCP server – `mcp/servers/skill_executor/server.py`**: Run CI summary scripts or flaky-test detectors through MCP orchestration.
