...
- **Python CLI – `scripts/list-skills`**: Chain reviews with experiment analysis or quality reports to maintain a single optimization narrative.
- **Sample artifact – `samples/whatsup-logistics/optimization/metric_review.md`**: Follow the WhatsUp Logistics review format for WhatsApp courier KPIs.
- **External API – Mixpanel & Google Analytics Data API**: Pull real-time performance slices to embed in the review.
- **MCP server – `mcp/servers/skill_executor/server.py`**: Run SQL or Python notebooks that produce supplementary charts through remote agents.
