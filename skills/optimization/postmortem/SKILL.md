...
- **Python CLI – `scripts/list-skills`**: Surface related governance and delivery skills to update (skill_release, test_plan) after the incident review.
- **Sample artifact – `samples/whatsup-logistics/optimization/postmortem.md`**: Follow the WhatsUp Logistics structure for timeline capture and corrective actions.
- **External API – PagerDuty & Slack Export APIs**: Retrieve incident timelines and responder chat logs to populate the chronology.
- **MCP server – `mcp/servers/skill_executor/server.py`**: Run log-scraping or metrics-diff scripts via MCP during analysis.
