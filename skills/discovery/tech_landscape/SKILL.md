...
- **Python CLI – `scripts/list-skills`**: Surface adjacent engineering discovery skills before kicking off landscape analysis.
- **Sample artifact – `samples/whatsup-logistics/discovery/tech_landscape.md`**: Borrow integration tables and spike formatting aligned to the WhatsUp Logistics initiative.
- **External API – GitHub & npm registries**: Query repository activity and package health to inform build-versus-buy trade-offs.
- **MCP server – `mcp/servers/skills_catalog/server.py`**: Provide remote engineers the same landscape prompt for asynchronous contributions or validation.
