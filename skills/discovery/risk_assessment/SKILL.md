...
- **Python CLI – `scripts/list-skills`**: Align on the full risk toolkit before entering discovery reviews.
- **Sample artifact – `samples/whatsup-logistics/discovery/risk_assessment.md`**: Mirror the risk taxonomy and mitigation layout proven in the WhatsUp Logistics sample.
- **External API – ServiceNow/GRC REST endpoints**: Pull outstanding compliance or security findings relevant to the scoped feature.
- **MCP server – `mcp/servers/skills_catalog/server.py`**: Broadcast risk definitions to distributed agents so mitigation tracking stays consistent.
