...
- **Python CLI – `scripts/list-skills`**: Remind the team of complementary research prompts (e.g., problem framing) before facilitation.
- **Sample artifact – `samples/whatsup-logistics/discovery/user_research.md`**: Adapt the WhatsUp Logistics brief to accelerate planning and logistics sections.
- **External API – WhatsApp Business Cloud API**: Coordinate recruit outreach and confirmation messages directly within the WhatsApp-first workflow.
- **MCP server – `mcp/servers/skills_catalog/server.py`**: Enable design partners working in IDEs to pull the same research blueprint via MCP requests.
